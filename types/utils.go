...
func stringHave0xPrefix(input string) bool {
	return len(input) >= 2 && input[0] == '0' && (input[1] == 'x' || input[1] == 'X')
}
...
func DecodeBig(input string) (*big.Int, error) {
	raw, err := checkNumberByString(input)
	if err != nil {
		return nil, err
	}
	if len(raw) > 64 {
...
func checkNumberByString(input string) (raw string, err error) {
	if len(input) == 0 {
		return "", ErrEmptyString
	}
	if !stringHave0xPrefix(input) {
...
func checkNumberByBytes(input []byte) (raw []byte, err error) {
	if len(input) == 0 {
		return nil, nil // empty strings are allowed
	}
	if !bytesHave0xPrefix(input) {
